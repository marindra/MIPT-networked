...
void send_time(ENetPeer *peer, uint32_t curTime)
{
  ENetPacket *packet = enet_packet_create(nullptr, sizeof(uint8_t) + sizeof(uint32_t), ENET_PACKET_FLAG_RELIABLE);
  uint8_t *ptr = packet->data;
  *ptr = E_SERVER_TO_CLIENT_SET_TIME; ptr += sizeof(uint8_t);
  memcpy(ptr, &curTime, sizeof(uint32_t)); ptr += sizeof(uint32_t);
...
void send_snapshot(ENetPeer *peer, uint16_t eid, float x, float y, float ori, uint32_t timestamp)
{
  ENetPacket *packet = enet_packet_create(nullptr, sizeof(uint8_t) + sizeof(uint16_t) +
  ...
  *ptr = E_SERVER_TO_CLIENT_SNAPSHOT; ptr += sizeof(uint8_t);
  ...
  memcpy(ptr, &timestamp, sizeof(uint32_t)); ptr += sizeof(uint32_t);
...
void deserialize_time(ENetPacket *packet, uint32_t &curTime)
{
  uint8_t *ptr = packet->data; ptr += sizeof(uint8_t);
  curTime = *(uint32_t*)(ptr); ptr += sizeof(uint32_t);
}
...
void deserialize_snapshot(ENetPacket *packet, uint16_t &eid, float &x, float &y, float &ori, uint32_t &timestamp)
{
  ...
  timestamp = *(uint32_t*)(ptr); ptr += sizeof(uint32_t);
}
